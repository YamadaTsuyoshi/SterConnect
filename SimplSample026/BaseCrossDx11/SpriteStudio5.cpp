	...
	*/
	...
	void SSPart::OnCreate(){
		try{
			...
			m_PtrObj->m_MeshRes = pImpl->m_SquareRes;
			m_PtrObj->m_TextureRes = pImpl->m_TextureResource;
			m_PtrObj->m_WorldMatrix = Mat4x4();
			m_PtrObj->m_Camera = GetStage<Stage>()->GetCamera();
			m_PtrObj->m_SamplerState = SamplerState::LinearClamp;
			//透明処理をする
			SetAlphaActive(true);
		}
		catch (...){
